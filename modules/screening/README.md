...
| Verdict | Exit | Means |
|---|---|---|
| `COPYABLE` | **0** | Clears the venue's 10 USDC floor at this ticket, and nothing else is worth reserving about. |
...
| `NOT COPYABLE` | **2** | Does not clear the floor. The member cannot place this trader's orders at this ticket, whatever else is true — impossible beats expensive. Any reservations are still printed below. |
...
| `extrapolation` | The venue could not return the whole window, and the monthly figures are stretched by **more than 3×**. Past **30×** there is no verdict at all. |
